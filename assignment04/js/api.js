...
function postArticle(req, res) {
    ...
    if (credentials && credentials.name === username && compare(credentials.pass, password)) {
        ...
        const createArticleQuery = `
            mutation CreateArticle($title: String!, $content: RichTextAST!, $category: Category!, $source: String!) {
                __typename
                createArticle(data: {title: $title, content: $content, category: $category, source: $source}) {
                    id,
                    title,
                    category,
                    content {
